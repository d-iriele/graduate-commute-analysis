# Findings Summary: The Cost of a Commute — Graduate Purchasing Power Across UK Cities
...
Graduate job offers are typically compared on salary alone, but this ignores two costs that vary enormously by location: the cost of commuting into the role, and the cost of housing near it. This project builds a rigorous, city-by-city comparison of graduate purchasing power across six UK cities — London, Manchester, Birmingham, Bristol, Leeds, and Edinburgh — incorporating salary, rent, commuting cost, and commuting time, to determine where a graduate's money and time actually go furthest.
...
- **Salary is measured at regional, not city, level** (LEO Graduate Outcomes data), with each city treated as the economic focal point of its region. Results should be read as a regional purchasing-power comparison anchored on major cities, not a claim about salaries specific to each named city alone.
- **Salary reflects earnings five years post-graduation**, not entry-level graduate salary — this dataset does not capture first-job earnings specifically.
- **One commute cost figure (Birmingham) was originally estimated**, not directly sourced, due to the Network West Midlands zonal fare system not returning a standard annual season ticket price via the National Rail calculator. A sensitivity check (see Finding 5) confirmed this estimate does not materially affect the project's conclusions.
- **Edinburgh's commute route is an intercity pattern** (Glasgow–Edinburgh) rather than the suburban commute pattern used for the other five cities, reflecting a genuinely common graduate living pattern in Scotland but not directly comparable in structure to the other five routes.
- **Scotland's rent data uses a different collection methodology** (Broad Rental Market Areas rather than local authorities), and ONS explicitly advises caution comparing Scotland's rent estimates against the other UK nations on a like-for-like basis.
- **All commute cost and time figures are based on a single representative route per city**, not the full distribution of possible commutes a graduate might realistically choose.